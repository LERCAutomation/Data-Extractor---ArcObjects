...
namespace HLESRISQLServerFunctions
{
    ...
    public class SQLServerFunctions
    {
        ...
        public string GetStringValue(ref SqlConnection aConnection, string aQuery)
        {
            // Note aQuery should return a single value in a single field, something like
            // "Select ColumnName from Table where UniqueID = 1"
            SqlCommand objCommand = new SqlCommand(aQuery, aConnection);
            string strResult = (string)objCommand.ExecuteScalar();
            return strResult;
        }
    }
}
