...
namespace DataExtractor3
{
    public partial class frmDataExtractor : Form
    {
        ...
        public frmDataExtractor()
        {
            ...
            // Get all the custom objects initialised.
            myConfig = new DataExtractorConfig(); // This calls all the Config functions and we can find out whether it's been successful.
            myFileFuncs = new FileFunctions();

            IApplication pApp = ArcMap.Application;
            myArcMapFuncs = new ArcMapFunctions(pApp);

            //// Get the relevant info from the Config file.
            ...
            if (blOpenForm)
            {
                ...
                if (!myArcMapFuncs.TableExists(strSDE + "\\" + strPartnerTable))
                {
                ...
                List<string> liExistingPartnerColumns = myArcMapFuncs.FieldsExist(strSDE, strPartnerTable, liAllPartnerColumns);
            ...
            if (blOpenForm)
            {
                ...
                ITable tblPartnerTable = myArcMapFuncs.GetTable(wsSQLWorkspace, strPartnerTable);
            ...
            if (blOpenForm)
            {
                ...
                if (liMissingLayers.Count == liAllLayers.Count) // There are no layers loaded.
                {
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            {
                IFeatureClass pFC = myArcMapFuncs.GetFeatureClass(strSDEName, strPartnerTable);
            ...
            foreach (string strPartner in liChosenPartners)
            {
                ...
                {
                    string strCheckTable = myConfig.GetDatabaseSchema() + "." + strPartnerTable;
